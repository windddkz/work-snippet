...
// Docker认证服务器地址
const dockerio_auth_url = 'https://auth.docker.io';
...
export default {
    ...
    const workers_hostname = url.hostname;
		...
		// 处理token请求
		if (hub_host == 'registry-1.docker.io' && url.pathname.includes('/token')) {
			let token_parameter = {
				headers: {
					'Host': 'auth.docker.io',
		...
		if (
			...
			if (repo) {
				const tokenUrl = `${dockerio_auth_url}/token?service=registry.docker.io&scope=repository:${repo}:pull`;
				...
				if (new_response_headers.get("Www-Authenticate")) {
					...
					let re = new RegExp(dockerio_auth_url, 'g');
